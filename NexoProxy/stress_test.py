...
for i in range(0, 1000):
    ...
    response = requests.post('http://192.168.1.154:3000/state', json={'entities': [{'name': 'l.bad1', 'state': _state}, {'name': 'l.bad11', 'state': _state}]})
    response = requests.post('http://192.168.1.154:3000/state', json={'entities': [{'name': 'l.gang1', 'state': _state}, {'name': 'l.gang11', 'state': _state}]})
    response = requests.post('http://192.168.1.154:3000/state', json={'entities': [{'name': 'l.korridor1', 'state': _state}, {'name': 'l.korridor11', 'state': _state}]})
    ...
    print(response.json())
    time.sleep(delay)
    #response = requests.post('http://192.168.1.154:3000/state/l.korridor11', json={'state': _state})
    #print(response.json())
    time.sleep(delay)
