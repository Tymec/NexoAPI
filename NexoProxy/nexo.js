...
class NexoJS {
      ...
      client.socket.on("data", (data) => {
        ...
        if (data_str.includes("Welcome")) {
          ...
          if (data_str.split(":")[1] === "")
            ...
            console.log("[NEXO] Server responded");
          }
        } else {
  ...
  auth() {
    ...
    if (!client.pass) console.log("No password specified.");
  ...
  checkConnection() {
    ...
    if (!client.isBusy) client.send("ping");
  }
  ...
  getState(name) {
    ...
    return new Promise(async (resolve, reject) => {
      await client
        ._sendAndRead(`system C '${name}' ?`)
        .then(async (success) => {
          if (success === "CMD OK") {
            await client._sendAndRead("get").then((data) => {
              client.busyTimer = setTimeout(() => {
                client.isBusy = false;
              }, 1000);
              client.processPoll();

              let splitData = data.split(":");
              if (splitData[1] === "") reject(`Invalid entity: ${name}`);
  ...
  async processPoll() {
    ...
    for (let i in poll) {
      await client
        ._sendAndRead(`system C '${poll[i]["name"]}' ${poll[i]["value"]}`)
        .then((success) => {
          if (success === "CMD OK") {
            const index = client.poll.indexOf(poll[i]);
            if (index > -1) {
              client.poll.splice(index, 1);
            }
          }
        });
    }
  ...
  setState(name, value) {
    ...
    return new Promise(async (resolve, reject) => {
      if (client.isBusy) {
        client.poll.push({ name: name, value: value });
        resolve();
      }
